        ...
        if _G.Details_Downtime then
            ...
            for _, Actor in Container:ListActors() do
                if Actor:IsGroupPlayer() then
                    ...
                    local target, hit = _G.Details_Downtime:GetRecord(CombatNumber, Actor:guid())
...
EO.CustomDisplayOverall = {
    ...
    total_script = [[
        ...
        if _G.Details_Downtime then
            return _G.Details_Downtime:GetDisplayText(Details:GetCombat(-1):GetCombatNumber(), Actor.my_actor:guid())
        end
        return ""
    ]],
}
...
do
    ...
    function EO:InstallPlugin()
        ...
        Details:InstallPlugin(
            'TOOLBAR', L["Downtime"], 133911, plugin, 'DETAILS_PLUGIN_DOWNTIME',
            MINIMAL_DETAILS_VERSION_REQUIRED, 'Bernycinders-Thrall', version, defaults
        )
